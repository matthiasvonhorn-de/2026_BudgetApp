...
export const PUT = withHandler(async (request: Request, ctx) => {
  ...
  const existing = await prisma.portfolioValue.findUnique({ where: { id: valueId } })
  if (!existing || existing.portfolioId !== id) throw new DomainError('Not found', 404)
  ...
  if (data.date !== undefined) {
    ...
    const existingAtDate = await prisma.portfolioValue.findUnique({
      where: {
        portfolioId_date: {
          portfolioId: id,
          date: dateOnly,
  ...
  const updated = await prisma.portfolioValue.update({
    where: { id: valueId },
    data: {
